...
class GraphModule(torch.nn.Module):
    def forward(self, primals_1: "f32[8]"):
        sin: "f32[8]" = torch.ops.aten.sin.default(primals_1)
        ...
        mul: "f32[8]" = torch.ops.aten.mul.Tensor(gt, sin)
        mul_1: "f32[8]" = torch.ops.aten.mul.Tensor(mul, 2.0);  mul = None
        add: "f32[8]" = torch.ops.aten.add.Tensor(mul_1, sin);  mul_1 = sin = None
        return (add, primals_1, gt)""",
                ignore_empty_lines=True,
            )
...
class GraphModule(torch.nn.Module):
    def forward(self, L_x_: "f32[8]"):
        ...
        getitem_1: "f32[8]" = tag_activation_checkpoint[0];  tag_activation_checkpoint = None

        detach: "f32[8]" = getitem_1.detach();  getitem_1 = None
        return (detach,)
...
class GraphModule(torch.nn.Module):
    def forward(self, L_x_: "f32[8]", L_y_: "f32[8]"):
        ...
        mul: "f32[8]" = torch.mul(l_x_, l_y_);  l_x_ = None
        mul_1: "f32[8]" = mul * 2;  mul = None
        mul_2: "f32[8]" = torch.mul(mul_1, l_y_);  mul_1 = l_y_ = None
        mul_3: "f32[8]" = mul_2 * 3;  mul_2 = None
        return (mul_3,)
...
""",
    ...
    def test_side_effect_with_aliased_intermediate(self):
        ...
        with self.assertRaisesRegex(
            ...
            "does not belong to this Graph|KeyError",
        ):
            opt_fn(x)
...
class GraphModule(torch.nn.Module):
    def forward(self, primals_1: "f32[8, 8]", primals_2: "f32[8, 8]"):
        ...
        sin_1: "f32[8, 8]" = torch.ops.aten.sin.default(sin);  sin = None

        t: "f32[8, 8]" = torch.ops.aten.t.default(primals_1);  primals_1 = None
        t_1: "f32[8, 8]" = torch.ops.aten.t.default(primals_2);  primals_2 = None
        return (sin_1, mm, t, t_1)
""",
            )
            ...
            self.assertExpectedInline(
                ...
                """\
class GraphModule(torch.nn.Module):
    def forward(self, mm: "f32[8, 8]", t: "f32[8, 8]", t_1: "f32[8, 8]", tangents_1: "f32[8, 8]"):
        sin: "f32[8, 8]" = torch.ops.aten.sin.default(mm)

        cos: "f32[8, 8]" = torch.ops.aten.cos.default(sin);  sin = None
        mul: "f32[8, 8]" = torch.ops.aten.mul.Tensor(tangents_1, cos);  tangents_1 = cos = None

        cos_1: "f32[8, 8]" = torch.ops.aten.cos.default(mm);  mm = None
        mul_1: "f32[8, 8]" = torch.ops.aten.mul.Tensor(mul, cos_1);  mul = cos_1 = None

        mm_1: "f32[8, 8]" = torch.ops.aten.mm.default(t, mul_1);  t = None
        mm_2: "f32[8, 8]" = torch.ops.aten.mm.default(mul_1, t_1);  mul_1 = t_1 = None
        return (mm_2, mm_1)
""",
            )
...
class GraphModule(torch.nn.Module):
    def forward(self, L_x_: "f32[8, 8]", L_y_: "f32[16, 16]"):
        ...
        sin: "f32[8, 8]" = torch.sin(l_x_);  l_x_ = None
        sin_1: "f32[16, 16]" = torch.sin(l_y_);  l_y_ = None
        return (sin, sin_1)
...
class GraphModule(torch.nn.Module):
    def forward(self, L_x_: "f32[s77, 8]", s77: "Sym(s77)"):
        ...
        floordiv: "Sym((s77//2))" = s77 // 2;  s77 = None

        sin: "f32[s77, 8]" = torch.sin(l_x_);  l_x_ = None

        narrow: "f32[(s77//2), 8]" = torch.narrow(sin, 0, 0, floordiv);  sin = floordiv = None

        sin_1: "f32[(s77//2), 8]" = torch.sin(narrow);  narrow = None
        return (sin_1,)
...
class GraphModule(torch.nn.Module):
    def forward(self, x):
        ...
        subgraph_2 = self.subgraph_0_0
        invoke_subgraph_2 = torch.ops.higher_order.invoke_subgraph(subgraph_2, 'subgraph_0', l_x_);  subgraph_2 = l_x_ = None
        a: "f32[1]" = invoke_subgraph_2[0];  a = None
        b: "i64[1]" = invoke_subgraph_2[1];  invoke_subgraph_2 = None

        getitem: "i64[1]" = invoke_subgraph[1];  invoke_subgraph = None
        return self._dynamo_bytecode_unflatten((getitem, b,), _fn_args)
